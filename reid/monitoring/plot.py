...
# ─── axis-fill functions ──────────────────────────────────────────────────────
# Each function receives an ax and fills it.
# The dashboard reuses them directly — no duplication between individual plots
# and the 2×3 grid.
...
def _ax_loss(ax, df):
    ...
    ax.axhline(0, color="gray", ls=":", lw=1, alpha=0.5)  # loss can go negative (uniformity term)
    ax.set(title="Triplet Loss", xlabel="Epoch", ylabel="Loss",
           xlim=(1, None), ylim=(None, None))
    ax.legend()
...
def _ax_triplet_gap(ax, df, margin: float = 0.3):
    ...
    ax.fill_between(                                           # only when order is correct
        e, df["th_mean_d_pos"], df["th_mean_d_neg"],
        where=df["th_mean_d_neg"] > df["th_mean_d_pos"],
        alpha=0.08, color=C_GREEN,
    )
    ax.axhline(0,      color="gray",   ls=":",  lw=1,   alpha=0.5)   # zero reference for gap
    ax.axhline(margin, color=C_ORANGE, ls="--", lw=1.5, alpha=0.8,
               label=f"margin {margin}")
    ax.set(title="Embedding Distances", xlabel="Epoch",
           ylabel="Distance", xlim=(1, None), ylim=(None, None))
    ax.legend(fontsize=8)
...
def _ax_grad_norms(ax, df):
    ...
    for i, col in enumerate(cols):
        ...
        ax.semilogy(e, df[col], color=color, lw=1.5, alpha=0.85,
...
def generate_all_plots(run_dir: str, margin: float = 0.3) -> None:
    ...
    Args:
        ...
        margin  : triplet margin — used as reference line in triplet_gap plot
    """
    ...
    with plt.rc_context(STYLE):
        ...
        fig.suptitle(f"Training Dashboard — {run_name}",
        ...
        plt.tight_layout()
        fig.savefig(os.path.join(plots_dir, "00_dashboard.png"),
