...
Uniformity loss (Wang et al., 2020):
  ...
  lambda_unif = 0.05 by default — small enough to not override the triplet
...
class BatchHardTripletLoss(nn.Module):
    ...
    def forward(
        ...
        sq_upper = dists.pow(2)[rows, cols] #(B*(B-1)/2,)
        unif_loss = torch.logsumexp(-2.0 * sq_upper, dim=0) - math.log(sq_upper.numel())  # scalar ≤ 0
